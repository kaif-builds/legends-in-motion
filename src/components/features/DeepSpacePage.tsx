...
/* ══════════════════════════════════════════════════════════════════
   DEEP SPACE PAGE
   Same Three.js canvas as TrophyPage (black + golden particles +
   Bloom). Hero orbs are pure-JS animated HTML elements with:
     · Z-depth controlling size / blur / glow / opacity
     · Unique sine-wave drift per star
     · Heavy-inertia mouse repulsion (scales with Z)
     · Scroll parallax (scales with Z)
   ══════════════════════════════════════════════════════════════════ */
...
interface StarDef {
  id: string;
  label: string;
  /** base position 0–100 % of viewport */
  bx: number;
  by: number;
  /** depth 0.2 (far) → 1.0 (close) */
  z: number;
  /** 0–1: deepSpaceProgress at which this star starts fading in */
  scrollThreshold: number;
  /** drift: unique per-axis frequency & amplitude */
  driftFreqX: number;
  driftFreqY: number;
  driftAmpX: number;
  driftAmpY: number;
  /** video clip to play when this star is clicked */
  videoSrc: string;
}
...
/** px diameter from Z */
const orbSize   = (z: number) => lerp(5,  18, z);   // 5 px far → 18 px close
/** blur from Z */
const orbBlur   = (z: number) => lerp(1.4, 0, z);   // blurry far → sharp close
/** glow halo radius from Z */
const orbHalo   = (z: number) => lerp(5,  16, z);   // px
/** outer opacity from Z */
const orbAlpha  = (z: number) => lerp(0.35, 1.0, z);
/** mouse repulsion reach (viewport %) from Z */
const mouseReach = (z: number) => lerp(6, 18, z);
/** mouse repulsion strength from Z */
const mouseStr   = (z: number) => lerp(0.008, 0.028, z);
/** scroll parallax multiplier from Z */
const scrollMul  = (z: number) => lerp(10, 60, z);  // vh
...
/* ═══════════════════════════════════════════════════════════════
   Hero Orb — all visibility driven by RAF loop via DOM refs
   ═══════════════════════════════════════════════════════════════ */
interface HeroOrbProps {
  star: StarDef;
  /** ref to the wrapper div (RAF sets transform) */
  orbRef: (el: HTMLDivElement | null) => void;
  /** ref to the core div (RAF sets opacity) */
  coreRef: (el: HTMLDivElement | null) => void;
  /** ref to the connector div (RAF sets opacity + line height) */
  connectorRef: (el: HTMLDivElement | null) => void;
  /** ref to the line div (RAF sets height) */
...
const HeroOrb: React.FC<HeroOrbProps> = ({
  ...
  return (
    ...
    >
      {/* Core — opacity starts at 0, RAF fades it in */}
      <div
        ref={coreRef}
      ...
      {/* Connector — opacity + line height driven by RAF */}
...
export const DeepSpacePage: React.FC<DeepSpacePageProps> = ({ scrollProgress }) => {
  ...
  const [zoomOrigin, setZoomOrigin] = useState<{
    px: number; py: number;   // pixel coords for transform-origin
    vx: number; vy: number;   // viewport-% for SpaceZoom bloom
    starColor: string;
    videoSrc: string;
  } | null>(null);
  const sceneRef = useRef<HTMLDivElement>(null);
  const zoomRaf = useRef<number>(0);
  // Ref mirror so the RAF animation loop can read phase without stale closures
  const zoomPhaseRef = useRef<'idle' | 'in' | 'hold' | 'out'>('idle');
  useEffect(() => { zoomPhaseRef.current = zoomPhase; }, [zoomPhase]);

  // Exponential ease: near-zero for first 40%, then slams forward
  ...
  const FWD_DURATION = 1400; // ms — zoom-in
  const REV_DURATION = 900;  // ms — zoom-out
  const MAX_SCALE = 25;
  ...
  const handleExplore = useCallback((id: string) => {
    ...
    // Get the star's current on-screen pixel position from its DOM element
  ...
  const handleZoomClose = useCallback(() => {
    // Read from ref — never stale, works from video `ended` event or button click
  ...
  /* ── Per-star smooth position offsets ── */
  ...
  useEffect(() => {
    const animate = (now: number) => {
      ...
      const sp = scrollRef.current; // 0–1 within deep space page
      ...
      STARS.forEach((star, i) => {
        ...
        /* ── 1. Scroll-based fade-in ──
           Star fades: scrollThreshold → scrollThreshold+0.10
           Line reveals: scrollThreshold+0.12 → scrollThreshold+0.22
        */
        ...
        // During zoom-in/hold/out: hide connector labels + lines instantly
  ...
  return (
    <div className="deep-space">
      {/* ── Scene layer: Canvas + orbs — this gets the zoom transform ── */}
      ...
      {/* Cinematic zoom overlay — bloom + video + close button.
          Lives OUTSIDE the transformed scene so position:fixed works. */}
